...
标签只能是以下之一：问候、退货、物流、支付、发票、非业务、需更新知识库。

判断规则：
- 如果用户的问题是店铺业务相关（退换货、物流、支付、发票等），但根据你的常识判断，知识库可能没有覆盖这个具体问题的答案，则输出"需更新知识库"。
- 例如："你们支持货到付款吗"这个问题，虽然属于"支付"类，但如果支付政策知识库里通常只列出支持的支付方式，可能没有明确写"不支持货到付款"，就应输出"需更新知识库"。
...
suggestion_agent = create_agent(
    model=llm,
    tools=[],
    system_prompt="""你是一个知识库管理专家。你已经被系统判定为需要更新知识库。

你的任务是：基于用户的问题和客服的回复，生成一条应该添加到知识库的Q&A条目。

输出格式：
Q：[用户的问题概括]
A：[建议的标准答案。如果客服回复中已包含可用信息，直接提取。如果客服回复不够完善，基于你的通用知识补充。]

注意：你被调用本身就说明知识库需要更新。不要输出"无需更新"。必须输出一条Q&A条目。
...
# 第二步：直接根据意图分类结果判断知识库是否覆盖
    knowledge_gap = (intent == "需更新知识库")
    ...
    # 第四步：如果知识库有盲区，强制触发 Agent 3
    if knowledge_gap:
        logger.info("检测到知识库盲区，强制触发知识更新建议Agent...")
        suggestion_input = f"用户消息：{user_input}\n客服回复：{reply}"
        result3 = suggestion_agent.invoke({"messages": [{"role": "user", "content": suggestion_input}]})
        suggestion = result3["messages"][-1].content
        
        from datetime import datetime
        with open("faq_suggestions.txt", "a", encoding="utf-8") as f:
            f.write(f"记录时间：{datetime.now()}\n")
            f.write(f"用户问题：{user_input}\n")
            f.write(f"更新建议：{suggestion}\n")
            f.write("---\n")
        logger.info("知识库更新建议已保存")
